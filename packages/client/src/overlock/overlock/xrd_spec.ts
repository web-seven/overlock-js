...
export interface XrdSpecProtoMsg {
  typeUrl: "/overlock.overlock.XrdSpec";
  value: Uint8Array;
}
/** Main CompositeResourceDefinition spec structure. */
export interface XrdSpecAmino {
  group?: string;
  names?: ResourceNamesAmino;
  versions?: VersionAmino[];
}
export interface XrdSpecAminoMsg {
  type: "/overlock.overlock.XrdSpec";
  value: XrdSpecAmino;
}
/** Main CompositeResourceDefinition spec structure. */
export interface XrdSpecSDKType {
  group: string;
...
function createBaseXrdSpec(): XrdSpec {
  return {
    group: "",
    names: undefined,
    versions: []
...
export const XrdSpec = {
  typeUrl: "/overlock.overlock.XrdSpec",
  encode(message: XrdSpec, writer: BinaryWriter = BinaryWriter.create()): BinaryWriter {
    if (message.group !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): XrdSpec {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseXrdSpec();
  ...
  fromPartial(object: Partial<XrdSpec>): XrdSpec {
    const message = createBaseXrdSpec();
  ...
  fromAmino(object: XrdSpecAmino): XrdSpec {
    const message = createBaseXrdSpec();
  ...
  toAmino(message: XrdSpec): XrdSpecAmino {
    const obj: any = {};
  ...
  fromAminoMsg(object: XrdSpecAminoMsg): XrdSpec {
    return XrdSpec.fromAmino(object.value);
  },
  fromProtoMsg(message: XrdSpecProtoMsg): XrdSpec {
    return XrdSpec.decode(message.value);
  },
  toProto(message: XrdSpec): Uint8Array {
    return XrdSpec.encode(message).finish();
  },
  toProtoMsg(message: XrdSpec): XrdSpecProtoMsg {
    return {
      typeUrl: "/overlock.overlock.XrdSpec",
      value: XrdSpec.encode(message).finish()
    };
  }
};
